-- Fix user profiles with correct enum type
-- This script creates the missing enum type and fixes user profiles
...
-- 3. Create a minimal profile insert that will work with any schema
DO $$
DECLARE
    v_columns TEXT;
    v_values TEXT;
    ...
    SELECT 
        ...
        string_agg(
            CASE column_name
                ...
                WHEN 'role' THEN quote_literal('ADMIN') || '::user_role_enum'
        ...
        BEGIN
            ...
            SELECT string_agg(
                CASE column_name
                    ...
                    WHEN 'role' THEN quote_literal(COALESCE(v_user.raw_user_meta_data->>'role', 'OPS')) || '::user_role_enum'
...
-- 5. Update the AuthProvider fallback function
    ...
    SELECT 
        ...
        string_agg(
            CASE 
                ...
                WHEN column_name = 'role' THEN quote_literal('OPS') || '::user_role_enum'
